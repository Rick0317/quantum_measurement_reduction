...
def main():
    ...
    if import_success and deps_success and func_success and ghost_success:
        ...
        print("\nExample usage:")
        print("from quantum_measurement_reduction.ghost_pauli import update_decomp_w_ghost_paulis_sparse")
        
    else:
        print("❌ SOME TESTS FAILED!")
        print("\nPlease check the following:")
        if not import_success:
            print("- Package installation (try: pip install git+https://github.com/Rick0317/quantum_measurement_reduction.git)")
        if not deps_success:
            print("- Dependencies installation (try: pip install numpy scipy openfermion qiskit cirq-core matplotlib)")
        if not func_success:
