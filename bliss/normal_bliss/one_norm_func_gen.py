...
def symmetric_matrix_from_upper_triangle(O, n):
    ...
    if len(O) != n * (n + 1) // 2:
        raise ValueError("Length of O must be n(n+1)/2 for an n x n symmetric matrix.")
...
def generate_analytical_one_norm(ferm_op, N, ne):
    ...
    majorana_terms = construct_majorana_terms(
        ferm_op, N, ne, x, y, z, w, O_matrix, O2_matrix
    )
...
def generate_analytical_one_norm_2_body(ferm_op, N, ne):
    ...
    for term, coeff_expr in majorana_terms.terms.items():
        ...
        row = [
            float(sp.re(coeffs_expanded.get(var, 0))) for var in T
    ...
    variable_bounds = [(None, None)] * n_vars + [
        (0, None)
    ] * n_constraints  # Auxiliary variables u_i ≥ 0

    # Solve LP using HiGHS solver
    res = linprog(c, A_ub=A_ub, b_ub=b_ub, bounds=variable_bounds, method="highs")
...
def generate_analytical_one_norm_3_body_cheap(ferm_op, N, ne):
    ...
    # Construct the Majorana terms manually
    majorana_terms = construct_majorana_terms_3_body_cheap(
        ferm_op, N, ne, z, O_matrix, O2_matrix
    )
    ...
    one_norm_func = sp.lambdify((z, O, O_2), one_norm_expr, modules=["numpy"])
...
def generate_analytical_one_norm_3_body(ferm_op, N, ne):
    ...
    # Construct the Majorana terms manually
    majorana_terms = construct_majorana_terms_3_body(
        ferm_op, N, ne, x, y, z, O_matrix, O2_matrix
    )
